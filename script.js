...
const DEFAULT_API_URL = `${BASE_URL}movie/now_playing?api_key=${API_KEY}&language=en-US&page=${page}`;
getMovies(DEFAULT_API_URL);

// ==============================================
// pagination
const prevPage = document.querySelector('#pre');
prevPage.addEventListener('click', handlePrevPage)
function handlePrevPage(){
  console.log(page)
    if(page <= 1){
       page =  1;
       getMovies(`${BASE_URL}movie/now_playing?api_key=${API_KEY}&language=en-US&page=${page}`);
    }else{
        page = page - 1;
        getMovies(`${BASE_URL}movie/now_playing?api_key=${API_KEY}&language=en-US&page=${page}`);
    }
...
searchBtn.addEventListener("click", () => {
  const prenext = document.querySelector('.pre-next').innerHTML = '';
